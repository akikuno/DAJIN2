...
def correct_cssplits(TEMPDIR: Path, FASTA_ALLELS: dict[str, str], CONTROL_NAME: str, SAMPLE_NAME: str) -> None:
    ...
    for allele, sequence in FASTA_ALLELS.items():
        ...
        transpose_mask_repdels_control = replace_repdels(transpose_control, repeat_dels)
        transpose_mask_repdels_sample = replace_repdels(transpose_sample, repeat_dels)
        # Distribute N
        transpose_mask_repdels_ends_n_control = replace_both_ends_n(transpose_mask_repdels_control)
        transpose_mask_repdels_ends_n_sample = replace_both_ends_n(transpose_mask_repdels_sample)
        # Replace CSSPLIT
        cssplits_mask_repdels_ends_n_control = [",".join(t) for t in transpose(transpose_mask_repdels_ends_n_control)]
        cssplits_mask_repdels_ends_n_sample = [",".join(t) for t in transpose(transpose_mask_repdels_ends_n_sample)]
        for i, cssplits in enumerate(cssplits_mask_repdels_ends_n_control):
            midsv_control[i]["CSSPLIT"] = cssplits
        for i, cssplits in enumerate(cssplits_mask_repdels_ends_n_sample):
