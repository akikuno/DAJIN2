...
def extract_different_loci(TEMPDIR, classif_sample, DICT_ALLELE, CONTROL_NAME):
    """
    ...
    for (ALLELE, SV), group in groupby(classif_sample, key=lambda x: (x["ALLELE"], x["SV"])):
        ...
        diffloci = screen_different_loci(cssplit_sample, cssplit_control, sequence, alpha=0.01, threshold=0.05)
