...
+ [ ] 🔴Stx2 `test_barcode25`において、`e0174c91bd7b`(大型欠失)がcontrolアレルへのマッピングでは両側のリードがアラインメントされているにもかからわず、deletionアレルへのマッピングでは片側しかアライメントされていない。このために大型欠失なのにcontrolアレルであると分類されてしまう。一方、同じ大型欠失の`ed618576505a`はcontrolでもdeletionでも両側のリードがアラインメントされている（これは想定通り）。
  + mappyにおいて、`-ax map-ont --splice`を使用する？
  + -> midsv自体を相当いじる必要があるが、価値はありそう…
    + 挿入（大小）、欠失（大小）、逆位、点変異に対して、`--splice`で作ったsamを用いてmidsvを作成する
