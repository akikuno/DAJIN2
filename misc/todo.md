...
+ [x] 問題：点変異の分類がうまくいっていない
  + 原因：点変異のようなごく少量のミスマッチは、ほかのシークエンスエラーに隠れて反映されにくい
  + 対策：FASTA_ALLELEにおいて、コントロールと違う塩基配列に対して重み付けをする
    + Mappyを用いて異なるFASTA_ALLELEの特徴的変異を抽出する（例：点変異部位）
    + 各リードにおいて特徴的変異をもつもののスコアを下げる（例：野生型アレルに対して点変異を持つリードはスコアを下げる）
  + 結果：`classify.py`に`extract_diff_loci`を入れ、アレルごとの特徴的な変異を抽出した
    + 少量の変異を対象とするため、配列全体の違いが10つ以下のアレルペアにおいてのみ抽出するようにした
    + 2023-01-12 Tyrで検証の結果、たしかに分類がされていた
