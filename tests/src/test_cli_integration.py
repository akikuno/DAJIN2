...
class TestCLIGenomeCoordinate:
    ...
    def test_cli_help_includes_bed_option(self):
        ...
        parser.add_argument("-g", "--genome", type=str, default="", help="Reference genome ID (e.g hg38, mm39) [default: '']")
        parser.add_argument("-b", "--bed", type=str, default="", dest="genome_coordinate", help="Path to BED6 file containing genomic coordinates [default: '']")
        
        help_text = parser.format_help()
        
        assert "-b, --bed" in help_text
    ...
    def test_cli_argparse_bed_short_option(self):
        ...
        bed_file = self.create_test_bed_file("chr1\t1000\t2000")
    ...
    @patch('DAJIN2.main.execute_single_mode')
    def test_cli_precedence_bed_over_genome(self, mock_execute):
        """Test that BED file takes precedence over genome ID in CLI processing."""
        from DAJIN2.main import execute
        
        bed_file = self.create_test_bed_file("chr1\t1000\t2000\ttest\t0\t+")
        ...
        try:
            with patch.object(sys, 'argv', test_args):
                with patch('DAJIN2.utils.input_validator.validate_files'):
                    with patch('DAJIN2.utils.input_validator.validate_bed_file_and_get_coordinates') as mock_bed_validate:
                        ...
                        # Verify execute_single_mode was called with BED coordinates
                        mock_execute.assert_called_once()
                        args_passed = mock_execute.call_args[0][0]
                        assert args_passed["genome_coordinate"] == bed_file
                        
        finally:
            Path(bed_file).unlink()

    @patch('DAJIN2.main.execute_single_mode')
    def test_cli_genome_only_when_no_bed(self, mock_execute):
        ...
        with patch.object(sys, 'argv', test_args):
            with patch('DAJIN2.utils.input_validator.validate_files'):
                with patch('DAJIN2.utils.input_validator.validate_genome_and_fetch_urls') as mock_genome_validate:
                    ...
                    # Verify execute_single_mode was called
                    mock_execute.assert_called_once()
                    args_passed = mock_execute.call_args[0][0]
                    assert args_passed["genome"] == "mm39"
                    assert args_passed["genome_coordinate"] == ""
