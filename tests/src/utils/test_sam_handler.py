...
def test_remove_overlapped_reads_real_sample():
    ...
    """
    sam = fileio.read_sam("tests/data/report/report_bam/remove_overlap.sam")
    sam = list(sam)
    test = remove_overlapped_reads(sam)
    answer = fileio.read_sam("tests/data/report/report_bam/answer.sam")
    answer = list(answer)
    assert test == answer
...
def test_remove_microhomology_ACGT():
    sam = fileio.read_sam("tests/data/report/microhomology/microhomology-ACGT.sam")
    sam = list(sam)
...
def test_remove_microhomology_insertion():
    sam = fileio.read_sam("tests/data/report/microhomology/microhomology-insertion.sam")
    sam = list(sam)
    test = remove_microhomology(sam)
...
def test_remove_microhomology_deletion():
    sam = fileio.read_sam("tests/data/report/microhomology/microhomology-deletion.sam")
    sam = list(sam)
    test = remove_microhomology(sam)
...
def test_remove_microhomology_overlapped_softclip():
    sam = fileio.read_sam("tests/data/report/report_bam/cables2_barcode34_microhomology.sam")
    sam = list(sam)
    test = remove_microhomology(sam)
    answer = list(fileio.read_sam("tests/data/report/report_bam/answer_cables2_barcode34_microhomology.sam"))
    assert test == answer


def test_remove_microhomology_real_singe_read():
    seq_id = "0ef147016ef0"
    sam = fileio.read_sam("tests/data/report/report_bam/barcode54_allele2_before.sam")
    sam = list(sam)
    ...
    test = remove_microhomology(sam)
    answer = fileio.read_sam(f"tests/data/report/report_bam/barcode54_allele2_{seq_id}_after.sam")
    answer = list(answer)
    assert test == answer
...
def test_remove_microhomology_real_all_500_reads():
    ...
    sam = list(sam)
    test = remove_microhomology(sam)
    answer = fileio.read_sam("tests/data/report/report_bam/barcode54_allele2_after.sam")
    answer = list(answer)
    assert test == answer
