...
from src.DAJIN2.utils import fileio


def test_sanitize_name_with_valid_path():
    assert fileio.sanitize_name("valid_name") == "valid_name"
    assert fileio.sanitize_name(Path("valid/name")) == "valid-name"


def test_sanitize_name_with_invalid_characters():
    assert fileio.sanitize_name("inva/lid:name?") == "inva-lid-name-"
    assert fileio.sanitize_name(Path("/inva>lid|name.")) == "-inva-lid-name-"


def test_sanitize_name_with_whitespace():
    assert fileio.sanitize_name("  leading space") == "leading-space"
    assert fileio.sanitize_name("trailing space ") == "trailing-space"


def test_sanitize_name_with_empty_string():
    with pytest.raises(ValueError) as e:
        fileio.sanitize_name(" ")
...
def test_convert_to_posix_winpath():
    ...
    test = fileio.convert_to_posix(path)
...
def test_write_jsonl():
    ...
    fileio.write_jsonl(file_path=test_filename, data=data_list)
...
def test_is_gzip_file(path_file, expected):
    assert fileio.is_gzip_file(path_file) == expected
