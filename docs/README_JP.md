...
DAJIN2は、ターゲットシーケンシングを用いた、ゲノム編集サンプルの遺伝型解析ツールです。

**DAJIN2**の名称は日本語の「一網**打尽**」に由来します。  
「意図した変異も意図しない変異も、一度に網羅的に検出する」という設計思想を表しています。  
...
+ **網羅的な変異検出**  
  ターゲット領域の解析を通じて、点変異から構造多型まで、あらゆるゲノム編集イベントを検出可能です。  
...
>[!NOTE]
> 前身の[DAJIN](https://github.com/akikuno/DAJIN)は深層学習を使用していたため、GPU付きのPCが必要でした。  
> 一方、**DAJIN2では深層学習を用いておらず、GPUは不要**です。  
> そのため、一般的なノートパソコンでも問題なく動作します。

>[!NOTE]
> macOS (Apple Sillicon)および Linux (x86_64)での実行を確認しています。  
...
>[!IMPORTANT]
> **Windowsユーザーの方へ**  
> DAJIN2はLinux環境での実行を前提としています。  
> Windowsをご利用の場合は、[WSL2（Windows Subsystem for Linux 2）](https://learn.microsoft.com/ja-jp/windows/wsl/install)を使用してください。

## [Bioconda](https://anaconda.org/bioconda/DAJIN2)（推奨）

Biocondaをご利用いただくには、conda環境のセットアップが必要です。  
そのため、[Miniforge](https://github.com/conda-forge/miniforge)のインストールを推奨します。  
...
>
> **2. 最新バージョンの確認**  
> https://github.com/akikuno/DAJIN2/releases/latest
> 
> **3. DAJIN2の再インストール (1と2が異なった場合) **
> ```bash
> conda deactivate
...
DAJIN2は、以下のコマンドでWebアプリを起動することができます：
